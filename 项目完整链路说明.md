...
- 步骤文件名靠语义
- 新增步骤时不需要重命名后续文件
- Hosted Checkout 作为 Plus PayPal 的短链路时，OAuth 尾部也必须保留 `post-login-phone-verification` / `post-bound-email-phone-verification` 节点，确保登录后进入 `add-phone / phone-verification` 时先走 HeroSMS / 5sim / NexSMS 接码，再进入 `confirm-oauth`。
