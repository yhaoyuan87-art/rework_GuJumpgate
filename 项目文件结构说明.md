...
- `tests/step-definitions-hosted-oauth-phone.test.js`：测试 Hosted Checkout 的 OAuth 尾部会在 `confirm-oauth` 前保留手机号验证节点，避免登录后进入 `add-phone` 时跳过 HeroSMS / 5sim / NexSMS 接码链路。
