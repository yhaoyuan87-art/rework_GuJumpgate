(function attachStepDefinitions(root, factory) {
  ...
  const FREE_OAUTH_PREFIX_STEP_DEFINITIONS = PLUS_PAYPAL_PREFIX_STEP_DEFINITIONS.slice(0, 5);
  ...
  const FREE_OAUTH_STEP_DEFINITIONS = createOpenAiSteps(FREE_OAUTH_PREFIX_STEP_DEFINITIONS, 6, 60, SIGNUP_METHOD_EMAIL);
  const FREE_OAUTH_PHONE_STEP_DEFINITIONS = createOpenAiSteps(FREE_OAUTH_PREFIX_STEP_DEFINITIONS, 6, 60, SIGNUP_METHOD_PHONE);
  const FREE_OAUTH_PHONE_BOUND_EMAIL_RELOGIN_STEP_DEFINITIONS = createOpenAiSteps(FREE_OAUTH_PREFIX_STEP_DEFINITIONS, 6, 60, SIGNUP_METHOD_PHONE, { phoneSignupReloginAfterBindEmailEnabled: true });
  ...
  function getOpenAiModeStepDefinitions(options = {}) {
    ...
    if (plusAccountAccessStrategy === PLUS_ACCOUNT_ACCESS_STRATEGY_FREE_OAUTH) {
      if (signupMethod === SIGNUP_METHOD_PHONE) {
        return reloginAfterBindEmail
          ? FREE_OAUTH_PHONE_BOUND_EMAIL_RELOGIN_STEP_DEFINITIONS
          : FREE_OAUTH_PHONE_STEP_DEFINITIONS;
      }
      return FREE_OAUTH_STEP_DEFINITIONS;
    }
    if (!isPlusModeEnabled(options)) {
      if (signupMethod === SIGNUP_METHOD_PHONE) {
        return reloginAfterBindEmail
  ...
  return {
    ...
    PLUS_ACCOUNT_ACCESS_STRATEGY_FREE_OAUTH,
