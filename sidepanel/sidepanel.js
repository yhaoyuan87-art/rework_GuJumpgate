...
const ACCOUNT_ACCESS_STRATEGY_UI_FREE_OAUTH = 'free_oauth';
...
const HERO_SMS_WEB_COUNTRY_FALLBACK_ITEMS = Object.freeze(
  '151:Chile|33:Colombia|73:Brazil|16:United Kingdom|31:South Africa|6:Indonesia|4:Philippines|41:Cameroon|52:Thailand|187:USA|117:Portugal|7:Malaysia|182:Japan|10:Vietnam|50:Austria|25:Laos|19:Nigeria|48:Netherlands|13:Israel|39:Argentina|2:Kazakhstan|54:Mexico|175:Australia|78:France|129:Greece|43:Germany|36:Canada|15:Poland|14:Hong Kong|105:Ecuador|49:Latvia|37:Morocco|196:Singapore|21:Egypt|24:Cambodia|53:Saudi Arabia|108:Bosnia|109:Dominican Republic|65:Peru|32:Romania|111:Qatar|163:Finland|62:Turkey|173:Switzerland|76:Angola|86:Italy|128:Georgia|46:Sweden|83:Bulgaria|22:India|74:Afghanistan|64:Sri Lanka|58:Algeria|172:Denmark|1:Ukraine|23:Ireland|56:Spain|99:Togo|8:Kenya|38:Ghana|66:Pakistan|67:New Zealand|119:Burundi|148:Armenia|26:Haiti|135:Liberia|70:Venezuela|92:Bolivia|156:Uruguay|29:Serbia|51:Belarus|59:Slovenia|91:Timor-Leste|143:Tajikistan|84:Hungary|140:Rwanda|34:Estonia|85:Moldova|177:South Sudan|27:Ivory Coast|82:Belgium|18:DR Congo|47:Iraq|5:Myanmar|63:Czech|77:Cyprus|94:Guatemala|96:Zimbabwe|139:Niger|87:Paraguay|88:Honduras|130:Guinea-Bissau|141:Slovakia|152:Burkina Faso|154:Gabon|155:Albania|174:Norway|68:Guinea|100:Kuwait|101:Salvador|102:Libya|103:Jamaica|104:Trinidad and Tobago|106:Swaziland|107:Oman|110:Syria|112:Panama|113:Cuba|114:Mauritania|115:Sierra Leone|116:Jordan|118:Barbados|11:Kyrgyzstan|120:Benin|121:Brunei|122:Bahamas|123:Botswana|124:Belize|125:Central African Republic|126:Dominica|127:Grenada|131:Guyana|132:Iceland|133:Comoros|134:Saint Kitts and Nevis|136:Lesotho|137:Malawi|138:Namibia|142:Suriname|144:Monaco|145:Bahrain|146:Reunion|147:Zambia|149:Somalia|150:Congo|153:Lebanon|157:Mauritius|158:Bhutan|159:Maldives|160:Guadeloupe|161:Turkmenistan|162:French Guiana|164:Saint Lucia|165:Luxembourg|166:Saint Vincent and the Grenadines|167:Equatorial Guinea|168:Djibouti|169:Antigua and Barbuda|170:Cayman Islands|171:Montenegro|176:Eritrea|178:Sao Tome and Principe|179:Aruba|17:Madagascar|180:Montserrat|181:Anguilla|183:North Macedonia|184:Seychelles|185:New Caledonia|186:Cape Verde|188:Palestine|189:Fiji|198:Samoa|199:Malta|201:Gibraltar|203:Kosovo|204:Niue|20:Macao|28:Gambia|30:Yemen|35:Azerbaijan|40:Uzbekistan|42:Chad|44:Lithuania|45:Croatia|55:Taiwan|57:Iran|60:Bangladesh|61:Senegal|69:Mali|71:Ethiopia|72:Mongolia|75:Uganda|79:Papua|80:Mozambique|81:Nepal|89:Tunisia|90:Nicaragua|93:Costa Rica|95:UAE|97:Puerto Rico|98:Sudan'
    .split('|')
    .map((entry) => {
      const [rawId, ...nameParts] = entry.split(':');
      return { id: Number(rawId), eng: nameParts.join(':') };
    })
    .filter((entry) => Number.isFinite(entry.id) && entry.id > 0 && entry.eng)
);
...
const HERO_SMS_COUNTRY_CN_NAME_OVERRIDES = Object.freeze({
  bahamas: '巴哈马',
  bosnia: '波黑',
  'cape verde': '佛得角',
  congo: '刚果',
...
const HERO_SMS_COUNTRY_CN_DISPLAY_NAMES = (() => {
  if (typeof Intl === 'undefined' || typeof Intl.DisplayNames !== 'function') {
    return null;
  }
  try {
...
function inferHeroSmsCountryChineseName(country = {}) {
  const existingChinese = String(country?.chn || '').trim();
  if (existingChinese) {
    return existingChinese;
  }
  ...
  for (const candidate of candidates) {
    const key = normalizeHeroSmsCountryAliasKey(candidate);
    if (!key) {
      continue;
    }
    const override = HERO_SMS_COUNTRY_CN_NAME_OVERRIDES[key];
    if (override) {
      return override;
    }
    const code = HERO_SMS_COUNTRY_ISO_CODE_BY_NAME.get(key);
    if (code && HERO_SMS_COUNTRY_CN_DISPLAY_NAMES) {
      const localized = String(HERO_SMS_COUNTRY_CN_DISPLAY_NAMES.of(code) || '').trim();
...
function buildHeroSmsCountryDisplayLabel(country = {}) {
  ...
  const chinese = inferHeroSmsCountryChineseName(country);
  if (chinese && english) {
    if (chinese.toLowerCase() === english.toLowerCase()) {
      return english;
    }
...
function applyHeroSmsFallbackSelection(countries = [], options = {}) {
  ...
  const selectedIds = normalized
    .map((entry) => {
      const provider = getSelectedPhoneSmsProvider();
      const id = normalizePhoneSmsCountryId(entry.id, provider);
      if (
        provider === PHONE_SMS_PROVIDER_HERO
        && String(id) === '151'
        && /japan|日本/i.test(String(entry.label || entry.countryLabel || ''))
      ) {
        return 182;
      }
      return id;
...
async function loadHeroSmsCountries() {
  ...
  if (provider === PHONE_SMS_PROVIDER_FIVE_SIM) {
    ...
    for (let page = 1; page <= 30; page += 1) {
      const url = new URL('https://hero-sms.com/api/v1/left-menu/service/dr/countries');
      ...
      const payload = await response.json();
      const pageCountries = Array.isArray(payload?.data) ? payload.data : [];
      countries.push(...pageCountries);
      if (!payload?.hasMore || !pageCountries.length) {
        break;
      }
    }
    clearTimeout(timeoutId);
    ...
    const optionItems = countries
      .filter((item) => Number(item?.id) > 0 && String(item?.name || item?.eng || '').trim())
      .map((item) => {
        const id = normalizeHeroSmsCountryId(item.id);
        const english = String(item.name || item.eng || '').trim();
        const fallback = HERO_SMS_SUPPORTED_COUNTRY_ITEMS.find(
          (country) => String(country.eng || '').toLowerCase() === english.toLowerCase()
        );
        const countryForLabel = {
          ...item,
          eng: english,
          chn: fallback?.chn || item?.chn || '',
        };
        const label = buildHeroSmsCountryDisplayLabel(countryForLabel);
        return {
          id,
          label: String(label || '').trim() || `Country #${id}`,
          searchText: [
            buildHeroSmsCountrySearchText(countryForLabel, label, String(id)),
            item.priceMinAvailable !== undefined ? `price ${item.priceMinAvailable}` : '',
            item.count !== undefined ? `count ${item.count}` : '',
          ].filter(Boolean).join(' '),
        };
      })
      .filter((item) => {
        if (!item.id || seenCountryIds.has(String(item.id))) {
          return false;
        }
        seenCountryIds.add(String(item.id));
        return true;
      });
    if (!optionItems.length) {
      throw new Error('国家列表为空');
    ...
    const fallbackItems = HERO_SMS_WEB_COUNTRY_FALLBACK_ITEMS
      .map((item) => {
        const id = normalizeHeroSmsCountryId(item.id);
        const english = String(item.eng || item.name || '').trim();
        const supported = HERO_SMS_SUPPORTED_COUNTRY_ITEMS.find(
          (country) => String(country.eng || '').toLowerCase() === english.toLowerCase()
        );
        const countryForLabel = { ...item, chn: supported?.chn || item.chn || '', eng: english };
        const label = buildHeroSmsCountryDisplayLabel(countryForLabel);
        return {
          id,
          label: String(label || english || '').trim() || `Country #${id}`,
          searchText: buildHeroSmsCountrySearchText(
            countryForLabel,
            label || english,
            String(id)
          ),
  ...
  const remapLegacyHeroSmsCountryId = (id) => {
    const normalizedId = String(id || '').trim();
    if (
      provider === PHONE_SMS_PROVIDER_HERO
      && normalizedId === '151'
      && /japan|日本/i.test(String(latestState?.heroSmsCountryLabel || ''))
    ) {
      return '182';
    }
    return normalizedId;
  ...
  const normalizedSelectedIds = previousSelectedIds
    .map((id) => remapLegacyHeroSmsCountryId(id))
...
async function previewHeroSmsPriceTiers() {
  ...
  for (const provider of providerOrder) {
    ...
    const maxPrice = priceRange.maxPrice;
    const preferredPrice = normalizeHeroSmsMaxPriceValue(inputHeroSmsPreferredPrice?.value || '');
    const apiKey = String(inputHeroSmsApiKey?.value || '').trim();

    const heroLines = ['HeroSMS:'];
    if (preferredPrice) {
      heroLines.push(`已指定实际拿号档位：${preferredPrice}（查询结果仅作接口参考）`);
    }
    if (priceRange.invalid) {
    ...
    for (const country of candidates) {
      ...
      try {
        try {
          const webOfferUrl = new URL(`https://hero-sms.com/api/v1/left-menu/service/dr/country/${countryId}/offers`);
          const webOfferResponse = await fetch(webOfferUrl.toString(), {
            cache: 'no-store',
            headers: { Accept: 'application/json' },
          });
          if (!webOfferResponse.ok) {
            throw new Error(`HTTP ${webOfferResponse.status}`);
          }
          const webOfferPayload = await webOfferResponse.json();
          const webOfferRoot = webOfferPayload?.data?.dr
            || (
              webOfferPayload?.data
              && typeof webOfferPayload.data === 'object'
              && !Array.isArray(webOfferPayload.data)
                ? Object.values(webOfferPayload.data)[0]
                : null
            );
          const tierStockByPrice = new Map();
          (Array.isArray(webOfferRoot?.operators) ? webOfferRoot.operators : []).forEach((operator) => {
            const freePriceOffers = operator?.freePriceOffers
              && typeof operator.freePriceOffers === 'object'
              && !Array.isArray(operator.freePriceOffers)
                ? operator.freePriceOffers
                : {};
            Object.entries(freePriceOffers).forEach(([rawPrice, rawCount]) => {
              const cost = Math.round(Number(rawPrice) * 10000) / 10000;
              const stockCount = Math.max(0, Math.floor(Number(rawCount) || 0));
              if (!Number.isFinite(cost) || cost <= 0) {
                return;
              }
          ...
          const webFreePrice = webOfferRoot?.freePrice && typeof webOfferRoot.freePrice === 'object'
            ? webOfferRoot.freePrice
            : {};
          const unlockedFloorCandidates = [
            webOfferRoot?.userPrice,
            webFreePrice.min,
            webFreePrice.default,
          ]
            .map((value) => Math.round(Number(value) * 10000) / 10000)
            .filter((value) => Number.isFinite(value) && value > 0);
          const unlockedFloor = unlockedFloorCandidates.length
            ? Math.max(...unlockedFloorCandidates)
            : null;
          ['min', 'default', 'maxAvailable'].forEach((key) => {
            const cost = Math.round(Number(webFreePrice[key]) * 10000) / 10000;
            if (!Number.isFinite(cost) || cost <= 0 || tierStockByPrice.has(cost)) {
              return;
            }
            tierStockByPrice.set(cost, 0);
          });
          ...
          const unlockedTierEntries = tierEntries.filter((entry) => (
            unlockedFloor === null || Number(entry.price) >= unlockedFloor
          ));
          const lockedTierEntries = tierEntries.filter((entry) => (
            unlockedFloor !== null
            && Number(entry.price) > 0
            && Number(entry.price) < unlockedFloor
          ));
          const unlockedPrices = unlockedTierEntries.map((entry) => entry.price);
          const inStockPrices = unlockedTierEntries
            .filter((entry) => Number(entry.count) > 0)
            .map((entry) => entry.price);
          const rangeAllPrices = filterPhoneSmsPriceValuesForPreviewRange(unlockedPrices, priceRange);
          const rangeInStockPrices = filterPhoneSmsPriceValuesForPreviewRange(inStockPrices, priceRange);
          const filteredTierEntries = filterPhoneSmsPriceEntriesForPreviewRange(unlockedTierEntries, priceRange);
          const tierPreviewText = formatPriceTiersWithZeroStockForPreview(filteredTierEntries, { maxPrice });
          const lockedTierText = lockedTierEntries.length
            ? formatPriceTiersWithZeroStockForPreview(lockedTierEntries, { maxPrice: null })
            : '';
          if (!rangeInStockPrices.length) {
            if ((priceRange.hasMinPrice || priceRange.hasMaxPrice) && unlockedPrices.length && !rangeAllPrices.length) {
              heroLines.push(`${countryLabel}: 网页购买报价 ${buildPhoneSmsPriceRangePreviewMessage(priceRange)}`);
              continue;
            }
            if (unlockedPrices.length) {
              const lowestKnown = rangeAllPrices.length ? rangeAllPrices[0] : unlockedPrices[0];
              heroLines.push(
                `${countryLabel}: 网页购买报价全档位暂无库存（当前可买最低标价 ${formatHeroSmsPriceForPreview(lowestKnown) || lowestKnown}）`
                + `${tierPreviewText ? `；报价：${tierPreviewText}` : ''}`
                + `${lockedTierText ? `；等级价需解锁：${lockedTierText}` : ''}`
              );
              continue;
            }
            throw new Error('网页报价为空');
          }
          ...
          const lowestLabel = priceRange.hasMinPrice || priceRange.hasMaxPrice ? '网页区间内可买最低' : '网页可买最低';
          const maxAvailable = formatHeroSmsPriceForPreview(webFreePrice.maxAvailable);
          const avgPrice = formatHeroSmsPriceForPreview(webFreePrice.avg);
          heroLines.push(
            `${countryLabel}: ${lowestLabel} ${lowestText}`
            + `${maxAvailable ? `；网页最高可用 ${maxAvailable}` : ''}`
            + `${avgPrice ? `；近一小时均价 ${avgPrice}` : ''}`
            + `${tierPreviewText ? `；报价：${tierPreviewText}` : ''}`
            + `${lockedTierText ? `；等级价需解锁：${lockedTierText}` : ''}`
          );
          continue;
        } catch (webOfferError) {
          console.warn('HeroSMS 网页报价查询失败，降级使用兼容 API：', webOfferError);
        }
...
function normalizePlusAccountAccessStrategy(value = '') {
  ...
  if (normalized === PLUS_ACCOUNT_ACCESS_STRATEGY_FREE_OAUTH) {
    return PLUS_ACCOUNT_ACCESS_STRATEGY_FREE_OAUTH;
  }
...
function normalizeAccountAccessStrategyUiValue(value = '') {
  ...
  if (normalized === ACCOUNT_ACCESS_STRATEGY_UI_FREE_OAUTH) {
    return ACCOUNT_ACCESS_STRATEGY_UI_FREE_OAUTH;
  }
...
function getAccountAccessStrategyUiValueForState(state = latestState) {
  ...
  if (strategy === PLUS_ACCOUNT_ACCESS_STRATEGY_FREE_OAUTH) {
    return ACCOUNT_ACCESS_STRATEGY_UI_FREE_OAUTH;
  }
  return ACCOUNT_ACCESS_STRATEGY_UI_OAUTH;
}
...
function resolvePanelModeFromExportAndStrategy(exportTarget = '', strategyUiValue = '') {
  ...
  if (strategy === ACCOUNT_ACCESS_STRATEGY_UI_FREE_OAUTH) {
    return target;
  }
...
function updatePanelModeUI() {
  ...
  if (selectAccountAccessStrategy) {
    ...
    Array.from(selectAccountAccessStrategy.options || []).forEach((option) => {
      ...
      } else if (option.value === ACCOUNT_ACCESS_STRATEGY_UI_FREE_OAUTH) {
        const supported = availableStrategies.includes(PLUS_ACCOUNT_ACCESS_STRATEGY_FREE_OAUTH);
        option.disabled = !supported;
        option.hidden = !supported;
      } else {
        option.disabled = false;
